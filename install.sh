...
echo -e "${BLUE}========================================${RESET}"
echo -e "${BLUE}       CLOUDFLARE TUNNEL SETUP${RESET}"
echo -e "${BLUE}========================================${RESET}"
echo
...
if command -v cloudflared >/dev/null 2>&1; then
    ...
    if ! command -v apt-get >/dev/null 2>&1; then
        echo -e "${RED}This automatic installer currently supports Debian/Ubuntu (APT).${RESET}"
        exit 1
    fi
...
cloudflared --version
echo
echo -e "${YELLOW}[2/3] Cloudflare Tunnel token required.${RESET}"
...
echo
echo -e "${YELLOW}[3/3] Installing and starting Cloudflare service...${RESET}"

if [ "$(id -u)" -eq 0 ]; then
    cloudflared service install "$CF_TOKEN" || true
    systemctl enable cloudflared 2>/dev/null || true
    systemctl restart cloudflared 2>/dev/null || systemctl start cloudflared
else
...
if command -v systemctl >/dev/null 2>&1; then
    if [ "$(id -u)" -eq 0 ]; then
        systemctl --no-pager --full status cloudflared 2>/dev/null || true
    else
        sudo systemctl --no-pager --full status cloudflared 2>/dev/null || true
    fi
fi
